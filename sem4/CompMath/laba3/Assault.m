function result = Assault(polinom)
  result.a=[];
  result.b=[];
  border = borders(polinom);
  border(1) = border(1)-1;
  border(2) = border(2)+1;
  while border(1)<border(2)
    tmpb = border(1);
    border(1) = border(1)+1;
    if assault_method_inf(polinom,tmpb,border(1)) == 1
      result.a(end+1) = methodDihtomia(polinom, tmpb, border(1)); 
      t = [tmpb,border(1)];
      result.b=[result.b; t];
    elseif assault_method_inf(polinom,tmpb,border(1)) > 1
      b1=tmpb;
      b2=border(1);
      findbor = true;
      while findbor
        step = abs(b1-b2)/2;
        b2 = tmpb;
          for i=1:abs(tmpb-border(1))/step
            b1=b2;
            b2+=step;
            if assault_method_inf(polinom,b1,b2) == 1
              tmpb = b1;
              border(1) = b2;
              result.a(end+1) = methodDihtomia(polinom, tmpb, border(1));
              t = [tmpb,border(1)];
              result.b=[result.b; t];
              findbor = false;
            endif
          endfor
      endwhile
    endif
  endwhile
  
  function answer = assault_method_inf(polinom, a, b)
    function ans = count_differents(polinom)
      ans = 0;
      sign = polinom(1);
      for e = polinom(2:end)
        if (e != sign) && (e!=0)
          ans+=1;
          sign = e;
        endif
      endfor
    endfunction
    sign1 = sign(findPolinom(polinom, b));
    sign_1 = sign(findPolinom(polinom, a));
    polinom1 = dif(polinom);
    sign1 = [sign1, sign(findPolinom(polinom1, b))];
    sign_1 = [sign_1, sign(findPolinom(polinom1, a))];
    while length(polinom1) > 1
        tmp = -div_pol_into_pol(polinom, polinom1).b;
        polinom = polinom1;
        polinom1 = tmp;
        sign1 = [sign1, sign(findPolinom(polinom1, b))];
        sign_1 = [sign_1, sign(findPolinom(polinom1, a))];
    endwhile
  n1 = count_differents(sign1);
  n_1 = count_differents(sign_1);
  answer = (n_1 - n1);
  endfunction
endfunction
